...
export async function performAISearch(
  ...
  // FIX: Destructure series_info and map it to series to avoid property access error.
  const searchableVideos = allVideos.map(({ id, title, description, category, series_info }) => ({
    id,
    title,
    description,
    category,
