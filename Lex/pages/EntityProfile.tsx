...
const EntityProfile = () => {
    ...
    const { currentUser, getUserById, ecosystemEntities } = useAuth();
    
    // Use local state to manage entities so we can update the 'isClaimed' status client-side
    const [entities, setEntities] = useState<EcosystemEntity[]>(ecosystemEntities);
    const [isClaimModalOpen, setIsClaimModalOpen] = useState(false);

    // Sync local state if context data were to ever change (e.g. refresh)
    useEffect(() => {
        setEntities(ecosystemEntities);
    }, [ecosystemEntities]);
    ...
    const claimedByUser = useMemo(() => {
        ...
        return getUserById(entity.claimedByUserId);
    }, [entity, getUserById]);
