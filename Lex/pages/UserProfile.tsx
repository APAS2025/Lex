...
import type { User, ProfessionalTier, Badge, IconName, LexiconTerm } from '../types';
...
const ProfileHeader = ({ user, tier }: { user: User, tier?: ProfessionalTier }) => {
    const TierIcon = tier ? iconMap[tier.icon] : null;
    return (
...
const XPProgressBar = ({ user, currentTier, nextTier }: { user: User, currentTier?: ProfessionalTier, nextTier?: ProfessionalTier }) => {
...
const StatsBlock = ({ user }: { user: User }) => (
    ...
    return (
        <div className="group relative text-center flex flex-col items-center">
            <div className="relative w-20 h-20 rounded-full flex items-center justify-center ring-2 ring-slate-700/50 group-hover:ring-yellow-400 transition-all duration-300 transform group-hover:scale-110 overflow-hidden bg-gradient-to-br from-slate-700 to-slate-900 shadow-lg shadow-black/30">
                ...
                <BadgeIcon className="h-10 w-10 text-yellow-400 z-10" />
            </div>
...
const BadgeCase = ({ user }: { user: User }) => (
    <div className="bg-slate-800/50 rounded-xl ring-1 ring-white/10 p-6">
        ...
        {user.badges.length > 0 ? (
            ...
            <div className="text-center text-slate-400 py-8">
                <Icons.TrophyIcon className="h-12 w-12 mx-auto text-slate-600 mb-2"/>
...
const ActivityFeed = ({ user, terms }: { user: User; terms: LexiconTerm[] }) => {
    // For mock purposes, find all comments by this user
    const userComments = useMemo(() => {
        // FIX: Check for term.comments existence before filtering.
                ...
                {userComments.length > 0 ? userComments.slice(0, 5).map(comment => (
                     <Link to={`/term/${comment.termId}`} key={comment.id} className="block group">
                        <div className="flex items-start space-x-3">
                            <Icons.ChatBubbleLeftRightIcon className="h-5 w-5 mt-1 text-slate-500 shrink-0"/>
                            <div>
...
const UserProfile = () => {
    ...
    const { getUserById, terms } = useAuth();
        ...
        <div className="container mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                <div className="lg:col-span-3 space-y-8">
                    ...
                    <ActivityFeed user={user} terms={terms} />
                </div>
