...
import { droobiSessions } from '../data/mockData';
import LiveSessionBanner from '../components/LiveSessionBanner';
import SessionCard from '../components/SessionCard';
import OnDemandLibrary from '../components/OnDemandLibrary';

const DroobiTVSessions: React.FC = () => {
  const liveSession = useMemo(() => droobiSessions.find(s => s.isLive), []);
  const upcomingSessions = useMemo(() => droobiSessions.filter(s => !s.isLive).sort((a, b) => new Date(a.dateTime).getTime() - new Date(b.dateTime).getTime()), []);
