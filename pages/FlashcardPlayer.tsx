...
import { ChevronLeftIcon, ChevronRightIcon, ArrowUturnUpIcon, XIcon, PlayIcon, FireIcon, BookmarkSlashIcon } from '../components/Icons';
...
const FlashcardPlayer: React.FC = () => {
    ...
    const cardsInDeck = useMemo(() => {
        if (!deck) return [];
        return deck.cardIds.map(id => flashcards.find(c => c.id === id)).filter(Boolean);
    }, [deck]);

    const currentCard = cardsInDeck[currentCardIndex];
    const vendor = useMemo<Vendor | undefined>(() => {
        if (!currentCard?.back.vendorId) return undefined;
        return vendors.find(v => v.id === currentCard.back.vendorId);
    }, [currentCard]);
        ...
        <div className="fixed inset-0 bg-[#0F172A] z-50 flex flex-col p-4 sm:p-6 lg:p-8">
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="absolute w-full h-full backface-hidden bg-slate-800 rounded-2xl ring-1 ring-white/10 flex flex-col items-center justify-center p-8 text-center">
                                <p className="text-lg font-semibold text-blue-400">{currentCard.front.context}</p>
                                <h3 className="text-4xl font-extrabold text-white mt-2">{currentCard.front.title}</h3>
                            </div>
                            {/* Back */}
                            <div className="absolute w-full h-full backface-hidden bg-slate-800 rounded-2xl ring-1 ring-white/10 p-8 rotate-y-180 flex flex-col justify-center overflow-y-auto">
                                <p className="text-lg md:text-xl text-slate-300 text-center">{currentCard.back.definition}</p>
                                {vendor && (
                                ...
                                {currentCard.back.videoUrl && (
                                     <div className="mt-4 max-w-xs mx-auto w-full">
                                        ...
                                        <button 
                                            onClick={(e) => { e.stopPropagation(); handlePlayVideo(currentCard.back.videoUrl!); }} 
            ...
            {currentCard.back.videoUrl && (
                <VideoModal
                    ...
                    title={`Micro-Lesson: ${currentCard.front.title}`}
                />
            )}
        </div>
    );
